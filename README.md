...
- 内置 5 家客户、7 种纸张、5 台机台、8 个覆盖各状态的样例订单、排产计划与 8 张返工单（含近两个月历史返工损耗数据）
...
| 模块 | 功能 |
|------|------|
| 📊 生产看板 | 在制/逾期/返工/低库存统计卡；**返工损耗汇总**（可按时间段统计返工单数/数量/补投张数/损耗金额，支持按返工原因、机台分组与点击下钻）；交期三级预警（逾期、≤2天紧急、≤5天预警）；近7日机台负荷图；订单状态分布、机台概况、低库存清单，全部可点击下钻 |
...
| 🔧 返工跟踪 | 返工单全生命周期：待处理 → 返工中 → 已闭环；记录工序、**返工机台**、原因（色差/套印/划伤/装订/材料）、数量、责任人、问题描述与处理结果；登记**补投张数**后系统自动按订单用纸单价折算**损耗金额**（只读，不可手工篡改）；页内汇总可按时间段、原因、机台查询返工数量、补投张数与金额，并可从汇总行下钻明细 |
...
| `GET/POST /api/reworks/`、`PATCH /api/reworks/{id}/` | 返工单（支持 `status`/`reason`/`machine`/`start_date`/`end_date` 筛选；`machine=none` 查未登记机台的单据） |
| `GET /api/reworks/loss_summary/` | 返工损耗汇总：按 `start_date`/`end_date`（发现日期）过滤，返回合计及按原因、按机台分组的返工单数/数量/补投张数/损耗金额 |
