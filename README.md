...
- **产能**：可用机台（非维保）× 单台日产能 10,000 份；逐日累计两周内空闲产能（日产能 − 当日已排计划量），同客户在制订单未排产部分优先占用，覆盖本单需求的最早一天为产能可完成日；两周排不下则按剩余量与日产能估算顺延。评估既有订单时，本单自身排产按计划日期逐日累计（已完成的任务抵扣产量），**排产排到哪天、完工就算哪天**，未排产的缺口才由空闲产能兜底；
