...
- 内置 5 家客户、7 种纸张、5 台机台、8 个覆盖各状态的样例订单、排产计划、交接班记录与返工单
...
| 模块 | 功能 |
|------|------|
| 📊 生产看板 | 在制/逾期/返工/低库存统计卡 + 今日整体达成率；交期三级预警（逾期、≤2天紧急、≤5天预警）；近7日机台负荷图；订单状态分布、机台概况、低库存清单，全部可点击下钻 |
...
| 📝 交接班记录 | 按日期+班次一键生成交接记录，自动汇总当班各机台计划量/实际产量/完成率；值班人补录异常说明与交接事项；同一机台同日同班次唯一；重复生成按当前排产刷新产量，排产删除/改期的失效记录自动清理（无填写内容）或清零保留（已填内容），手动补录记录不受影响；异常判定统一按去首尾空白后的内容；顶部展示当日整体达成率与各班次日结 |
...
| `GET/POST /api/handovers/`、`PATCH/DELETE /api/handovers/{id}/` | 交接班记录（同机台同日同班次唯一） |
| `POST /api/handovers/generate/` | 按日期（可带班次）从排产汇总生成交接记录；失效记录自动清理或清零保留 |
| `GET /api/handovers/summary/?date=` | 当日整体达成率、分班次日结、异常机台数 |
