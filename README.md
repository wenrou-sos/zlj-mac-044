...
| 📝 交接班记录 | 按日期+班次一键生成交接记录，自动汇总当班各机台计划量/实际产量/完成率；值班人补录异常说明与交接事项；同一机台同日同班次仅一条（重复生成只刷新产量、保留已填内容）；支持无排产机台手动补录；顶部展示当日整体达成率与各班次日结 |
...
| `POST /api/handovers/generate/` | 按日期（可带班次）从排产汇总生成交接记录，已存在记录只刷新产量 |
