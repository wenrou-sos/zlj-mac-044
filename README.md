...
| 📋 订单管理 | 订单新建/编辑/筛选（关键字、状态、客户、交期级别）；列表内嵌三工序流水线、进度条与装订产量对账标记；详情抽屉维护工序进度、**各工序实际产量/合格数**、排产、返工 |
...
印前 / 印刷 / 装订三道工序除 0–100% 进度外，均可在"更新进度/登记产量"中登记**实际产量**与**合格数**：

- 工序由未开始转为开工（或直接完成）时，实际产量与合格数**默认取订单印数**，班组可按实际上报修改，也可一键"取订单印数"；
- 订单详情三张工序卡片连续展示 实际产量 / 合格数 / 损耗，损耗公式为 **实际产量 − 合格数 + 返工数量**，即返工数量（含已闭环）计入本工序损耗；
- 卡片与列表对印刷、装订产量给出"对印数多/少 N 份 / 与印数一致"标记；顶部提示条按**装订累计产量**与订单印数核对，多印或少印均会预警；
- 服务端校验：合格数不能大于实际产量、未开始工序不能登记产量（回到未开始自动清零）。
...
| `GET/PATCH /api/orders/{id}/progress/` | 三工序进度与产量（`*_actual_qty` 实际产量、`*_qualified_qty` 合格数；响应附带各工序返工数量 `*_rework_qty` 与损耗 `*_waste_qty`，服务端校验 0–100、状态一致性、合格数≤实际产量） |
