...
面向中小型印刷厂的生产管理系统，覆盖 **订单管理、纸张材料、机台排产、印前/印刷/装订工序进度、交期预警、返工跟踪、发货登记** 全流程。

- **后端**：Django 5 + Django REST Framework + SQLite
- **前端**：Vue 3 + Element Plus（无构建，本地静态资源，离线可用）
- 内置 5 家客户、7 种纸张、5 台机台、9 个覆盖各状态的样例订单、排产计划、返工单与发货记录
...
| 模块 | 功能 |
|------|------|
| 📊 生产看板 | 在制/待发货/已发货/逾期/返工/低库存统计卡；交期三级预警（逾期、≤2天紧急、≤5天预警）；待发货订单发货进度清单；近7日机台负荷图；订单状态分布、机台概况、低库存清单，全部可点击下钻 |
| 📋 订单管理 | 订单新建/编辑/筛选（关键字、状态、客户、交期级别）；列表内嵌三工序流水线、进度条与发货进度；详情抽屉维护工序进度、排产、返工、发货 |
| 🚚 发货登记 | 完工订单可分批登记发货，每批记录数量、发货日期、收货人、物流单号；累计发货超印数自动拦截；全部发完订单自动变为已发货，删除发货记录自动回退 |
...
- 更新工序进度后**订单主状态自动推导**（印前中 → 印刷中 → 装订中 → 待发货）；
- 登记返工单后，对应工序与订单自动进入 **返工中**；
- 返工闭环后工序自动恢复为进行中，订单回到实际工序；
- 全部工序完成自动记录完工日期，订单进入 **待发货**。

### 发货与订单状态联动

- 仅 **待发货**（已完工）订单可登记发货，支持分批，每批记录数量、发货日期、收货人、物流单号；
- 累计发货数量超过印数时服务端拦截并提示剩余可发数量；
- 全部发完订单自动变为 **已发货**；删除发货记录后自动回退为待发货；
- 已发货订单不再随工序变动（有未闭环返工单时除外）。

## 主要 API

| 接口 | 说明 |
|------|------|
| `GET /api/dashboard/` | 看板汇总（统计、预警订单、待发货清单、低库存、7日负荷） |
| `GET/POST /api/orders/`、`PATCH /api/orders/{id}/` | 订单 |
| `GET/PATCH /api/orders/{id}/progress/` | 三工序进度（服务端校验 0–100 与状态一致性） |
| `GET/POST/DELETE /api/shipments/` | 发货记录（可 `?order=` 过滤；超印数拦截；联动订单发货状态） |
...
├── factory/
│   ├── models.py           # 客户/纸张/机台/订单/工序进度/排产/返工/发货/流水
